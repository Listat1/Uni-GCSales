...
<section class="search-bar" id="searchBar">
    ...
    <button class="btn" id="browseBtn">Select Category</button>
</section>
...
<script>
    ...
    document.querySelectorAll('.cat-grid-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            const categoryName = e.target.innerText;

            // 1. Update the Hero Text (Replaces whatever was there)
            selectedCatDisplay.innerHTML = `<span style="color: #ffeb3b;">Focusing on:</span> ${categoryName}`;

            // 2. Visual feedback: Snap the Hero back to the splash/image state
            heroBanner.classList.remove('show-categories');
            browseBtn.classList.remove('btn-active');

            // 3. Update the Browse Button text to show we are filtering
            browseBtn.innerText = "Change Category";

            // 4. Trigger the product area to show it's filtering (Placeholder for now)
            productArea.innerHTML = `Showing items in <strong>${categoryName}</strong>...`;
        });
    });
